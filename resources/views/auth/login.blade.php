        ...
        <div class="lg:w-1/2 w-full flex items-center justify-center text-center md:px-16 px-0 z-0"
            ...
            <div class="w-full py-6 z-20">
                <h1 class="my-6">
                    ...
                    <img class="m-auto h-14 mt-2" src="{{asset('svg/logo_name.svg.png')}}" />
                </h1>
                <form method="post" action="{{route('login')}}" class="sm:w-2/3 w-full px-4 lg:px-0 mx-auto">
                    ...
                    <div class="pb-2 pt-4">
                        <input class="block w-full p-4 text-lg rounded-sm bg-black" type="password" name="password"
                               id="password" placeholder="{{__('Contraseña')}}" required autocomplete="current-password">
                        <div class="mt-3">{{$errors->first('username')}}</div>
                    </div>
                    <div class="px-4 pb-2 pt-4">
                        <button
                                class="uppercase block w-full p-4 text-lg rounded-full bg-red-500 hover:bg-red-600 focus:outline-none">{{__('Entrar')}}</button>
                    </div>
                </form>
            </div>
        </div>
